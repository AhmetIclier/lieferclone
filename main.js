...
function render() {
    ...
    for (let i = 0; i < dishes.length; i++) {
        dishbox.innerHTML += generateDishboxHtml(i);
    }
}

function generateDishboxHtml (i) {
...
function renderCheckout() {
    ...
    for (let i = 0; i < dishamount.length; i++) {
        if (dishamount[i] > 0) {
            ...
            cartbox.innerHTML += generateCheckoutHtml(i, zwischensumme);
        }
    }
    zwischensum();
    checkCartContent();
...
function generateCheckoutHtml(i, zwischensumme) {
    return `<div class="dish-in-cart" id="dish-num-check${i}">
...
function zwischensum() {
    ...
    checkMBW(zwischensumme);
}

function checkMBW (zwischensumme) {
    if (zwischensumme < 15) {
